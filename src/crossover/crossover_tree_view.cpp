                                               ...
                                               Glib::ustring const& new_text)
{
    ...
    if (!path.empty())
    {
...
#ifndef NDEBUG
        std::cout << "crossover_tree_view::on_cell_edited_value: path[0:1:2:4] = "
        ...
        // damping network...) we have Number of extra circuits + the crossover is the
        // total number of "base nodes" after filter type nodes
        ...
        if (!mod)
        {
            ...
            {
                net.parts()[path[3] + net.get_lowpass_order()].set_value(
                    row[m_columns.value]);
            }
        }
#ifndef NDEBUG
        std::cout << "crossover_tree_view::on_cell_edited_value: Id = "
                  << row[m_columns.id] << "\n";
#endif
...
{
    {
        ...
        Gtk::TreeViewColumn* column = m_TreeView.get_column(
                                               ...
                                               const Gtk::TreeModel::iterator& iter)
{
    ...
    renderer.property_text() = gspk::double_to_ustring((*iter)[m_columns.value], 3, 1);
