...
void Net::parse_lowpass_family(xmlNodePtr node)
{
    ...
    {
        throw std::runtime_error(_("Net::parse_lowpass_family: lowpass_family node "
                                   "expected"));
    }
}
...
void Net::parse_highpass_family(xmlNodePtr node)
{
    ...
    {
        throw std::runtime_error(_("Net::parse_highpass_family: highpass_family node "
                                   "expected"));
    }
}
...
void Net::parse_adv_imp_model(xmlNodePtr node)
{
    ...
    {
        throw std::runtime_error(_("Net::parse_adv_imp_model: adv_imp_model node "
                                   "expected"));
    }
}
...
auto Net::to_SPICE(Speaker const& s, bool const use_gnucap) -> std::string
{
...
auto operator<<(std::ostream& output, Net const& net) -> std::ostream&
{
    output << _("***Net*** ******") << "\n"
           << _("Id:       ") << net.m_id << "\n"
           << _("Type:     ") << net.m_type << "\n"
        ...
        if (diff > 0)
        {
            ...
            switch (diff)
            {
                case 1: // add one part
                    switch (m_lowpass_order)
                    {
                        // We will add the same part to a 0th order filter as we
                        // would add to a 2nd order filter
                        ...
                        case NET_ORDER_3RD:
                            ...
                            break;
                            // Also we can't add 1 part to a 4th order filter so
                            // we don't use its identifier here
                    }
                    break;
                case 2:
                    switch (m_lowpass_order)
                    {
                        case NET_NOT_PRESENT:
                        case NET_ORDER_2ND:
                            // Since we're inserting the parts...last one comes
                            // first in the std::vector, we insert them in reversed order
